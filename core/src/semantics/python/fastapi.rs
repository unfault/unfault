...
pub struct FastApiRoute {
    ...
    /// Used to resolve prefix when `include_router(router, prefix="/foo")` is present.
...
/// Extract HTTP method, path, and router var name from a route decorator.
///
/// Handles patterns like:
/// - `@app.get("/path")`
/// - `@router.post("/path")`
///
/// Returns `(http_method, path, router_var)`.
...
///
/// Handles `prefix="/assistant"` inside `include_router(router, prefix="/assistant")`.
fn extract_keyword_string_arg(file: &ParsedFile, args: Node, key: &str) -> Option<String> {
    ...
    for child in args.children(&mut cursor) {
        ...
        // keyword_argument has children: identifier "=" value
        let mut kw_cursor = child.walk();
        let kw_children: Vec<_> = child.children(&mut kw_cursor).collect();
        // First child is the keyword name, third is the value (after "=")
