...
"#,
        ...
        assert_eq!(
            findings.len(),
            3,
            "Expected 3 findings for GET, PUT, DELETE without logging"
        );

        // Verify all three methods are detected
        let detected_methods: Vec<&str> = findings
            .iter()
            .filter_map(|f| f.description.as_ref())
            .filter_map(|d| {
                if d.contains("`GET`") {
                    Some("GET")
                } else if d.contains("`PUT`") {
                    Some("PUT")
...
"#,
        ...
        // This test is purely for debugging output; there's nothing to assert.
    }
}
