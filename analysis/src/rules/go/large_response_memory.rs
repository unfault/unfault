        ...
        for (file_id, sem) in semantics {
            ...
            for call in &go.calls {
                if call.function_call.callee_expr.contains("json.Marshal") {
                    ...
                    let has_large_data_hint = call.args_repr.contains("[]")
                        ...
                        findings.push(RuleFinding {
                            ...
                            description: Some(
                                "json.Marshal loads the entire response into memory before \
                                 ...
                                 to stream the response directly to the client."
                                    .to_string(),
                            ),
                            ...
                            patch: Some(FilePatch {
                                ...
                                hunks: vec![PatchHunk {
                                    ...
                                    replacement: "// Stream JSON response instead of buffering:
                ...
                if call.function_call.callee_expr.contains("WriteFile") {
                    ...
                    findings.push(RuleFinding {
                        ...
                        patch: Some(FilePatch {
                            ...
                            hunks: vec![PatchHunk {
                                range: PatchRange::InsertBeforeLine { line },
                                replacement: "// Stream to file instead of buffering:
...
// return json.NewEncoder(f).Encode(data)"
            ...
            let has_rows_scan = go
                .calls
                .iter()
                .any(|c| c.function_call.callee_expr.contains("Scan"));
            let has_append_in_loop = go
                .calls
                .iter()
                .any(|c| c.function_call.callee_expr == "append" && c.in_loop);
            ...
            if has_rows_scan && has_append_in_loop && !has_pagination {
                ...
                if let Some(append_call) = go
                    .calls
                    .iter()
                    .find(|c| c.function_call.callee_expr == "append" && c.in_loop)
                {
                    ...
                    findings.push(RuleFinding {
                        ...
                        patch: Some(FilePatch {
                            ...
                            hunks: vec![PatchHunk {
                                range: PatchRange::InsertBeforeLine { line },
                                replacement: "// Add pagination to database queries:
