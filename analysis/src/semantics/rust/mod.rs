...
fn collect_semantics(parsed: &ParsedFile, sem: &mut RustFileSemantics) {
    // RustFileSemantics::from_parsed currently runs a lightweight collection in
    // `model.rs` (legacy). Ensure this pass doesn't accumulate duplicates.
    sem.uses.clear();
    sem.functions.clear();
    sem.structs.clear();
    sem.enums.clear();
    ...
    sem.statics.clear();
    sem.calls.clear();
    sem.field_accesses.clear();
    sem.variable_bindings.clear();
    sem.macro_invocations.clear();
    sem.sync_patterns.clear();

    sem.async_info = model::AsyncInfo::default();
...
fn update_context(
    ...
    match node.kind() {
        ...
        "static_item" | "const_item" => {
            // Any static/const initializer is executed once, so work performed here
            // is not in a hot path.
...
fn build_call_site(
    ...
    let function_call = FunctionCall {
        ...
        caller_qualified_name: ctx.current_function.clone().unwrap_or_default(),
...
fn walk_for_error_handling(
    ...
    // Look for ignored results (statement-position calls that might return Result/Option).
    //
...
"#;
    ...
    #[test]
    fn does_not_mark_if_let_as_ignored_result_statement() {
        let src = r#"
...
"#;
        ...
        // We should not flag the `if let` expression statement as an ignored fallible result.
        assert!(
            sem.result_ignores.is_empty(),
            "if-let control flow should not be treated as an ignored Result statement"
        );
    }
}
